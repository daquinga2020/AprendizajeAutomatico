...
class DogsDataset(Dataset):
    ...
    def __init__(self, root_dir, target_images_per_class = -1, transform=None):
        ...
        if self.target_images_per_class != -1:
            self.classes = self.classes[:self.target_images_per_class]
    ...
    def _load_dataset(self):
        ...
        for f in self.classes:
            ...
            img_path = os.path.join(self.root_dir, f)
            ...
            label = torch.tensor(self.classes.index(f), dtype=torch.long)
            for angle in range(0, 360, 3):
                rotation_matrix = cv2.getRotationMatrix2D((IMAGE_SIZE//2, IMAGE_SIZE//2), angle, 2.0)
                # Aplica la matriz de rotación a la imagen
                rotated_img = cv2.warpAffine(inv_img, rotation_matrix, (IMAGE_SIZE, IMAGE_SIZE))
                rotated_img = Image.fromarray(rotated_img)
                
                if self.transform:
                    rotated_img = self.transform(rotated_img)
            
                dataset.append((rotated_img, label))
            
        
        '''for class_folder in self.classes:
            class_path = os.path.join(self.root_dir, class_folder)
            class_images = [f for f in os.listdir(class_path) if f.endswith('.png')]
            ...
            for img_filename in class_images:
                ...
                for angle in range(0, 360, 3):
                    rotation_matrix = cv2.getRotationMatrix2D((IMAGE_SIZE//2, IMAGE_SIZE//2), angle, 1.0)
...
dataset_Dogs = DogsDataset(root_dir='pkmns_imgs', target_images_per_class=15, transform=transform)
...
pth2save = 'models/cnn_pkmns_3conv_50epch.pth'
torch.save(cnn_net, pth2save)
# cnn_net = torch.load(pth2save)
